...
approveForMyOrg() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode approveformyorg -o localhost:7050 --channelID $CHANNEL_NAME --name marbles02_private --version ${VERSION} --collections-config "../chaincode/marbles02_private/collections_config.json" --init-required --package-id ${PACKAGE_ID} --sequence ${VERSION} --waitForEvent >&log.txt
    set +x
  else
    set -x
...
checkCommitReadiness() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
    peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name marbles02_private --version ${VERSION} --collections-config "../chaincode/marbles02_private/collections_config.json" --sequence ${VERSION} --output json --init-required >&log.txt
    res=$?
    set +x
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ] ; then
    set -x
    peer lifecycle chaincode commit -o localhost:7050 --channelID $CHANNEL_NAME --name marbles02_private $PEER_CONN_PARMS --version ${VERSION} --collections-config "../chaincode/marbles02_private/collections_config.json" --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
...
chaincodeInvokeInit() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode invoke -o localhost:7050 -C $CHANNEL_NAME -n marbles02_private $PEER_CONN_PARMS --isInit -c '{"function":"init","Args":[]}' >&log.txt
    res=$?
    set +x
  else
    set -x
    peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n marbles02_private $PEER_CONN_PARMS --isInit -c '{"function":"init","Args":[]}' >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
chaincodeQuery() {
  ...
  # we either get a successful response, or reach MAX RETRY
    ...
    set -x
   peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n marbles02_private $PEER_CONN_PARMS -c '{"function":"initMarble","Args":[]}' --transient "{\"marble\":\"$MARBLE\"}" >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
